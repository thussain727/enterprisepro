...
<div class="childpagecontent"> <!-- Main content container holding page text and table -->
    ...
    //$sqlstatement = $conn1->prepare($sqlquery);
    //$sqlstatement->bind_param("ss", $school, $sourcetype);
    //$school = $_POST['School'];
    //$sourcetype = $_POST['Sourcetype'];
    //$results = $sqlstatement->execute();
    $results = $conn1->query($sqlquery);
    echo "<table class='sourcestable'>";
        ...
        echo "<tr>
            ...
            <td>" . $row["Notes"] . "</td>
            <td><a href='" . $row["Hyperlink"] . "'>" . $row["Hyperlink"] . "</a></td>
        </tr>";
        }
    echo "</table>";
    ?>
